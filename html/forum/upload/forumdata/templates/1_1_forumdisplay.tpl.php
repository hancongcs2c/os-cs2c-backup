...
0
|| checktplrefresh('/var/www/html/forum/upload/././templates/default/forumdisplay.htm', '/var/www/html/forum/upload/././templates/default/header.htm', 1378780358, '1', './templates/default')
|| checktplrefresh('/var/www/html/forum/upload/././templates/default/forumdisplay.htm', '/var/www/html/forum/upload/././templates/default/recommend.htm', 1378780358, '1', './templates/default')
|| checktplrefresh('/var/www/html/forum/upload/././templates/default/forumdisplay.htm', '/var/www/html/forum/upload/././templates/default/footer.htm', 1378780358, '1', './templates/default')
|| checktplrefresh('/var/www/html/forum/upload/././templates/default/forumdisplay.htm', '/var/www/html/forum/upload/././templates/default/jsmenu.htm', 1378780358, '1', './templates/default')
;?>
...
<p class="forumstats">[ <strong><?=$forum['threads']?></strong> Thread / <? echo $forum['posts']-$forum['threads']; ?> Reply ]</p>
<div class="forumaction">
<div class="right">
<?=$pluginhooks['forumdisplay_forumaction']?>
<? if($discuz_uid) { ?>
<a href="my.php?item=attention&amp;type=forum&amp;action=add&amp;fid=<?=$fid?>" id="ajax_attention" class="attention" onclick="ajaxmenu(this);doane(event);">focus</a>
<a href="my.php?item=favorites&amp;fid=<?=$fid?>" id="ajax_favorite" onclick="ajaxmenu(this);doane(event);">Favorite</a>
<? } if($rssstatus) { ?><a href="rss.php?fid=<?=$fid?>&amp;auth=<?=$rssauth?>" target="_blank" class="feed">RSS</a><? } ?>
</div>
<? if($forum['recyclebin'] && $forum['ismoderator']) { ?>
<a href="<? if($adminid == 1) { ?>admincp.php?action=recyclebin&amp;frames=yes<? } elseif($forum['ismoderator']) { ?>modcp.php?action=recyclebins&amp;fid=<?=$fid?><? } ?>" class="recyclebin" target="_blank">Recyclebin</a>
<? } ?>
</div>
<? if($forum['description']) { ?><p class="channelinfo">Forum Description: <?=$forum['description']?></p><? } ?>
<p id="modedby">
<? if($forum['ismoderator']) { ?>
<div class="modlink">
<?=$pluginhooks['forumdisplay_modlink']?>
<? if($forum['modworks']) { if($reportnum) { ?><a href="modcp.php?action=report&amp;fid=<?=$fid?>" target="_blank">User report(<?=$reportnum?>)</a> |<? } if($modnum) { ?><a href="modcp.php?action=moderate&amp;op=threads&amp;fid=<?=$fid?>" target="_blank">There are some admin works in waiting in this board (<?=$modnum?>)</a> |<? } if($modusernum) { ?><a href="modcp.php?action=moderate&amp;op=members&amp;fid=<?=$fid?>" target="_blank">There are some admin works on this user (<?=$modusernum?>)</a> |<? } } ?>
<a href="modcp.php?fid=<?=$fid?>" target="_blank">Moderator CP</a>
</div>
<? } ?>
Moderated by: <? if($moderatedby) { ?><?=$moderatedby?><? } else { ?>*Opening*<? } ?>
</p>
</div>
...
<div class="list">
<span class="headactions"><img onclick="toggle_collapse('modarea_c');" alt="Close/Open" title="Close/Open" src="<?=IMGDIR?>/<?=$collapseimg['modarea_c']?>.gif" id="modarea_c_img" class="toggle" /></span>
<h3>
<? if($forum['recommendlist']) { ?>
<a href="javascript:;" id="tab_1" class="current" <? if($forum['rules']) { ?> onclick="switchTab('tab', 1, 2)"<? } ?>>Recommended</a><? if($forum['ismoderator'] && $allowrecommendthread) { ?><a href="modcp.php?action=forums&amp;op=recommend&amp;show=all&amp;fid=<?=$fid?>" target="_blank">[Edit]</a><? } } if($forum['recommendlist'] &&  $forum['rules']) { ?><span class="pipe">|</span><? } if($forum['rules']) { ?><a href="javascript:;" id="tab_2"<? if(!$forum['recommendlist']) { ?> class="current"<? } ?> <? if($forum['recommendlist']) { ?> onclick="switchTab('tab', 2, 2)"<? } ?>>Forum Rules</a><? } ?>
</h3>
</div>
...
<?=$multipage?>
<span <? if($visitedforums) { ?>id="visitedforums" onmouseover="$('visitedforums').id = 'visitedforumstmp';this.id = 'visitedforums';showMenu({'ctrlid':this.id})"<? } ?> class="pageback"><a href="<?=$indexname?>">Back HOME</a></span>
<span class="postbtn" id="newspecial" prompt="post_newthread" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"><a href="post.php?action=newthread&amp;fid=<?=$fid?>"<? if(!$forum['allowspecialonly']) { ?> onclick="showWindow('newthread', this.href);return false;"<? } ?>>New</a></span>
</div>
...
<p>
<? if($typeid || $sortid) { ?><a href="forumdisplay.php?fid=<?=$fid?>">All</a><? } else { ?><strong>All</strong><? } if(is_array($forum['threadtypes']['flat'])) { foreach($forum['threadtypes']['flat'] as $id => $name) { if($typeid != $id) { ?><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=type&amp;typeid=<?=$id?><?=$sortadd?>"><?=$name?></a><? } else { ?><strong><?=$name?></strong><? } ?> <? } } if($forum['threadtypes']['selectbox']) { ?>
<span id="threadtypesmenu" class="dropmenu" onmouseover="showMenu({'ctrlid':this.id})">More Category</span>
...
<p>
<? if(!$forum['threadtypes']['listable']) { if($sortid) { ?><a href="forumdisplay.php?fid=<?=$fid?>">All</a><? } else { ?><strong>All</strong><? } } if(is_array($forum['threadsorts']['flat'])) { foreach($forum['threadsorts']['flat'] as $id => $name) { if($sortid != $id) { ?><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=sort&amp;sortid=<?=$id?><?=$typeadd?>"><?=$name?></a><? } else { ?><strong><?=$name?></strong><? } ?> <? } } if($forum['threadsorts']['selectbox']) { ?>
<span id="threadsortsmenu" class="dropmenu" onmouseover="showMenu({'ctrlid':this.id})">More Category</span>
...
<tr>
<td colspan="2">&nbsp;<a href="javascript:;" id="filtertype" class="dropmenu" onclick="showMenu({'ctrlid':this.id});"><? if($filter == poll) { ?>Poll
<? } elseif($filter == trade) { ?>Trade
<? } elseif($filter == activity) { ?>Activity
<? } elseif($filter == debate) { ?>Debate
<? } elseif($filter == reward) { ?>Reward
<? } else { ?>Type<? } ?></a></td>
<? if($forum['ismoderator']) { ?><td class="icon">&nbsp;</td><? } ?>
<th>
<ul class="itemfilter s_clear">
<li>Thread:</li>
<li<? if(!$filter) { ?> class="current"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>"><span>All</span></a></li>
<li<? if($filter == 'digest') { ?> class="current"<? } ?>><a class="filter" href="forumdisplay.php?fid=<?=$fid?>&amp;filter=digest"><span>Digest</span></a></li>
<? if($recommendthread['status']) { ?><li<? if($filter == 'recommend') { ?> class="current"<? } ?>><a class="filter" href="forumdisplay.php?fid=<?=$fid?>&amp;filter=recommend&amp;orderby=recommends"><span>Recommended</span></a></li><? } ?>
<li class="pipe">|</li>
<li>Time:</li>
<li<? if($filter == 86400) { ?> class="current"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;orderby=<?=$orderby?>&amp;filter=86400<?=$typeadd?><?=$sortadd?>"><span>1 day</span></a></li>
<li<? if($filter == 172800) { ?> class="current"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;orderby=<?=$orderby?>&amp;filter=172800<?=$typeadd?><?=$sortadd?>"><span>2 days</span></a></li>
<li<? if($filter == 604800) { ?> class="current"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;orderby=<?=$orderby?>&amp;filter=604800<?=$typeadd?><?=$sortadd?>"><span>1 week</span></a></li>
<li<? if($filter == 2592000) { ?> class="current"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;orderby=<?=$orderby?>&amp;filter=2592000<?=$typeadd?><?=$sortadd?>"><span>1 month</span></a></li>
<li<? if($filter == 7948800) { ?> class="current"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;orderby=<?=$orderby?>&amp;filter=7948800<?=$typeadd?><?=$sortadd?>"><span>3 months</span></a></li>
<li class="pipe">|</li>
<li><a class="order <? if($orderby == 'heats' || $orderby == 'recommends') { ?>order_active<? } ?>" href="forumdisplay.php?fid=<?=$fid?>&amp;filter=<?=$filter?>&amp;orderby=<? if($filter == 'recommend') { ?>recommends<? } else { ?>heats<? } ?><?=$typeadd?><?=$sortadd?>">Hot Topics</a></li>
</ul>
</th>
<td class="author"><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=<?=$filter?>&amp;orderby=dateline<?=$typeadd?><?=$sortadd?>" class="order <? if($orderby == 'dateline') { ?>order_active<? } ?>">Author/Time</a></td>
<td class="nums"><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=<?=$filter?>&amp;orderby=replies<?=$typeadd?><?=$sortadd?>" class="order <? if($orderby == 'replies') { ?>order_active<? } ?>">Replies</a>&nbsp;<a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=<?=$filter?>&amp;orderby=views<?=$typeadd?><?=$sortadd?>" class="order <? if($orderby == 'views') { ?>order_active<? } ?>">Views</a></td>
<td class="lastpost"><cite><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=<?=$filter?>&amp;orderby=lastpost<?=$typeadd?><?=$sortadd?>" class="order <? if($orderby == 'lastpost') { ?>order_active<? } ?>">Last Post</a></cite></td>
</tr>
</thead>
...
<tbody>
<tr>
<td class="folder"><img src="<?=IMGDIR?>/ann_icon.gif" alt="Note" /></td>
<td class="icon">&nbsp;</td>
<? if($forum['ismoderator']) { ?><td class="icon">&nbsp;</td><? } ?>
<th class="subject"><strong>Note: <? if(empty($announcement['type'])) { ?><a href="announcement.php?id=<?=$announcement['id']?>#<?=$announcement['id']?>" target="_blank"><?=$announcement['subject']?></a><? } else { ?><a href="<?=$announcement['message']?>" target="_blank"><?=$announcement['subject']?></a><? } ?></strong></th>
<td class="author">
...
<th class="subject">Forum Thread</th><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
</tr>
</tbody>
<? } ?>
<tbody id="<?=$thread['id']?>">
<tr>
<td class="folder">
<a href="viewthread.php?tid=<?=$thread['tid']?>&amp;extra=<?=$extra?>" title="Open in new window" target="_blank">
...
<td class="icon">
<? if($thread['special'] == 1) { ?>
<a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=poll"><img src="<?=IMGDIR?>/pollsmall.gif" alt="Poll" /></a>
<? } elseif($thread['special'] == 2) { ?>
<a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=trade"><img src="<?=IMGDIR?>/tradesmall.gif" alt="Trade" /></a>
<? } elseif($thread['special'] == 3) { ?>
<a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=reward"><img src="<?=IMGDIR?>/rewardsmall.gif" alt="Reward" <? if($thread['price'] < 0) { ?>class="solved"<? } ?> /></a>
<? } elseif($thread['special'] == 4) { ?>
<a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=activity"><img src="<?=IMGDIR?>/activitysmall.gif" alt="Activity" /></a>
<? } elseif($thread['special'] == 5) { ?>
<a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=debate"><img src="<?=IMGDIR?>/debatesmall.gif" alt="Debate" /></a>
<? } else { ?>
<?=$thread['icon']?>
<? } ?>
</td>
...
<a href="topicadmin.php?action=moderate&amp;optgroup=3&amp;operation=delete&amp;tid=<?=$thread['moved']?>" onclick="showWindow('mods', this.href);return false">Move:</a>
<? } else { ?>
Move:
<? } } ?>
...
<img src="<?=IMGDIR?>/recommend_<?=$thread['recommendicon']?>.gif" class="attach" alt="Recommend Index <?=$thread['recommends']?>" title="Recommend Index <?=$thread['recommends']?>" />
<? } if($thread['heatlevel']) { ?>
<img src="<?=IMGDIR?>/hot_<?=$thread['heatlevel']?>.gif" class="attach" alt="<?=$thread['heatlevel']?> Hot Topics" title="<?=$thread['heatlevel']?> Hot Topics" />
<? } if($thread['digest'] > 0) { ?>
<img src="<?=IMGDIR?>/digest_<?=$thread['digest']?>.gif" class="attach" alt="Digest <?=$thread['digest']?>" title="Digest <?=$thread['digest']?>" />
<? } if($thread['rate'] > 0) { ?>
<img src="<?=IMGDIR?>/agree.gif" class="attach" alt="Credit Added" title="Credit Added" />
<? } } if($thread['multipage']) { ?>
...
<a href="space.php?uid=<?=$thread['authorid']?>">Anonymous</a>
<? } else { ?>
Anonymous
<? } } ?>
</cite>
...
<td class="lastpost">
<cite><? if($thread['lastposter']) { ?><a href="<? if($thread['digest'] != -2) { ?>space.php?username=<?=$thread['lastposterenc']?><? } else { ?>viewthread.php?tid=<?=$thread['tid']?>&amp;page=<? echo max(1, $thread['pages']);; } ?>"><?=$thread['lastposter']?></a><? } else { ?>Anonymous<? } ?></cite>
...
<tbody><tr><th colspan="6"><p class="nodata">No Thread in this Forum or Range.</p></th></tr></tbody>
<? } ?>
</table>
...
<?=$multipage?>
<span <? if($visitedforums) { ?>id="visitedforums" onmouseover="$('visitedforums').id = 'visitedforumstmp';this.id = 'visitedforums';showMenu({'ctrlid':this.id})"<? } ?> class="pageback"><a href="<?=$indexname?>">Back HOME</a></span>
<span class="postbtn" id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"><a href="post.php?action=newthread&amp;fid=<?=$fid?>"<? if(!$forum['allowspecialonly']) { ?> onclick="showWindow('newthread', this.href);return false;"<? } ?>>New</a></span>
</div>
...
<h3>Active Users in this Forum</h3>
</dd>
<dd>
<ul class="s_clear"><? if(is_array($whosonline)) { foreach($whosonline as $key => $online) { ?><li title="Time: <?=$online['lastactivity']?><?="\n"?> Action: <?=$online['action']?><?="\n"?> Forum: <?=$forumname?>">
...
<li><a href="forumdisplay.php?fid=<?=$fid?>">All</a></li>
<? if($showpoll) { ?><li <? if($filter == 'poll') { ?>class="active"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=poll">Poll</a></li><? } if($showtrade) { ?><li <? if($filter == 'trade') { ?>class="active"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=trade">Trade</a></li><? } if($showreward) { ?><li <? if($filter == 'reward') { ?>class="active"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=reward">Reward</a></li><? } if($showactivity) { ?><li <? if($filter == 'activity') { ?>class="active"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=activity">Activity</a></li><? } if($showdebate) { ?><li <? if($filter == 'debate') { ?>class="active"<? } ?>><a href="forumdisplay.php?fid=<?=$fid?>&amp;filter=debate">Debate</a></li><? } if($threadplugins) { if(is_array($forum['threadplugin'])) { foreach($forum['threadplugin'] as $tpid) { if(array_key_exists($tpid, $threadplugins) && @in_array($tpid, $allowthreadplugin)) { ?>
