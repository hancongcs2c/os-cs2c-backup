...
0
|| checktplrefresh('/var/www/html/forum/upload/./templates/default/post.htm', '/var/www/html/forum/upload/./templates/default/header.htm', 1378869573, '2', './templates/colors')
|| checktplrefresh('/var/www/html/forum/upload/./templates/default/post.htm', '/var/www/html/forum/upload/./templates/default/post_editor.htm', 1378869573, '2', './templates/colors')
|| checktplrefresh('/var/www/html/forum/upload/./templates/default/post.htm', '/var/www/html/forum/upload/./templates/default/seccheck.htm', 1378869573, '2', './templates/colors')
|| checktplrefresh('/var/www/html/forum/upload/./templates/default/post.htm', '/var/www/html/forum/upload/./templates/default/post_editor_menu.htm', 1378869573, '2', './templates/colors')
|| checktplrefresh('/var/www/html/forum/upload/./templates/default/post.htm', '/var/www/html/forum/upload/./templates/default/footer.htm', 1378869573, '2', './templates/colors')
|| checktplrefresh('/var/www/html/forum/upload/./templates/default/post.htm', '/var/www/html/forum/upload/./templates/default/jsmenu.htm', 1378869573, '2', './templates/colors')
;?>
...
<? if($action == 'newthread' && $modnewthreads) { ?><em class="needverify">Approve</em><? } if($action == 'reply' && $modnewreplies) { ?><em class="needverify">Approve</em><? } ?><?
$creditstring = <<<EOF
...
?>
<? if($creditstring) { ?>
<span><a href="faq.php?action=credits&amp;fid=<?=$fid?>" target="_blank" title="Credits Policy"><? if($action == 'newthread') { ?>After you Post<? } elseif($action == 'reply') { ?>After you Reply<? } ?>&nbsp;<?=$creditstring?></a></span>
<? } ?>
</h3>
...
<? } else { ?>
<span id="subjecthide" class="left">RE: <?=$thread['subject']?> [<a href="javascript:;" onclick="display('subjecthide');display('subjectbox');$('subject').value='RE: <? echo htmlspecialchars(str_replace('\'', '\\\'', $thread['subject'])); ?>'">Modify</a>]</span>
...
<div class="right"><input type="checkbox" name="delete" id="delete" value="1" title="Delete Post<? if($thread['special'] == 3) { ?>, refund the reward price without the procedure fee.<? } ?>"><label for="delete">Del?</label></div>
<? } } ?>
<div class="left">
...
<? if(!$sortid) { ?><option value="0">Category</option><? } if(is_array($threadsorts['types'])) { foreach($threadsorts['types'] as $tsortid => $name) { if(!empty($modelid) && $threadsorts['modelid'][$tsortid] == $modelid || empty($modelid)) { ?>
...
<option value="0">Types</option><? if(is_array($forum['threadtypes']['types'])) { foreach($forum['threadtypes']['types'] as $typeid => $name) { ?><option value="<?=$typeid?>"<? if($thread['typeid'] == $typeid) { ?> selected="selected"<? } ?>><? echo strip_tags($name);; ?></option><? } } ?></select>
</div>
<? } ?>
</div>
</div>
...
<p class="left">Choose Viewpoint: </p>
<div class="float_typeid short_select">
<select id="stand" name="stand">
<option value="0">Neutral</option>
<option value="1"<? if($stand == 1) { ?> selected<? } ?>>Square</option>
<option value="2"<? if($stand == 2) { ?> selected<? } ?>>Opponet</option>
</select>
</div>
</div>
<? } if($showthreadsorts) { ?>
...
<div class="editor">
<a id="<?=$editorid?>_switcher" class="plugeditor editormode"><input type="checkbox" name="checkbox" value="0" <? if(!$editormode) { ?>checked="checked"<? } ?> onclick="switchEditor(this.checked?0:1)" />Code</a>
<div class="editorbtn">
<div class="maxbtn">
<a id="<?=$editorid?>_cmd_paste" title="Paste">Paste</a>
</div>
<div class="minibtn">
<p>
<a id="<?=$editorid?>_cmd_simple" title="Bold">B</a>
<a id="<?=$editorid?>_cmd_fontname" title="Font">Font</a>
<a id="<?=$editorid?>_cmd_fontsize" title="Size">Size</a>
<a id="<?=$editorid?>_cmd_forecolor" title="Forecolor">Color</a>
<a id="<?=$editorid?>_cmd_createlink" title="Insert URL">Url</a>
<em></em>
<a id="<?=$editorid?>_cmd_unlink" title="Remove URL">Unlink</a>
<a id="<?=$editorid?>_cmd_removeformat" title="Remove Format">Removeformat</a>
<a id="<?=$editorid?>_cmd_undo" title="UnDo">Undo</a>
<a id="<?=$editorid?>_cmd_redo" title="ReDo">Redo</a>
</p>
<p>
<a id="<?=$editorid?>_cmd_paragraph" title="Paragraph">P</a>
<a id="<?=$editorid?>_cmd_table" title="Insert Table">Table</a>
<a id="<?=$editorid?>_cmd_list" title="List">List</a>
<? if($allowhidecode) { ?>
<a id="<?=$editorid?>_cmd_hide" title="Insert Hide Information">Hide</a>
<? } else { ?>
<a id="<?=$editorid?>_cmd_hidden" title="Insert Hide Information" class="hidebtn_disabled">Hide</a>
<? } ?>
<a id="<?=$editorid?>_cmd_free" title="Insert Free Infomation">Free</a>
<em></em>
<a id="<?=$editorid?>_cmd_savedata" title="Save Data">Savedata</a>
<a id="<?=$editorid?>_cmd_loaddata" title="Restore">Loaddata</a>
<a id="<?=$editorid?>_cmd_clearcontent" title="Reset">Clearcontent</a>
<a id="<?=$editorid?>_cmd_checklength" title="Length Check">Checklength</a>
</p>
</div>
<div class="maxbtn">
<a id="<?=$editorid?>_cmd_smilies" title="Smilies">Smilies</a>
<div id="<?=$editorid?>_cmd_image_notice" class="haspic" style="display:none">!</div>
<a id="<?=$editorid?>_cmd_image" title="Insert Image URL">Insert Image URL</a>
<? if($allowpostattach) { ?>
<div id="<?=$editorid?>_cmd_attach_notice" class="hasatt" style="display:none">!</div>
<a id="<?=$editorid?>_cmd_attach" title="Attachment">Attachment</a>
<? } if($forum['allowmediacode']) { ?>
<a id="<?=$editorid?>_cmd_audio" title="Audio">Audio</a>
<a id="<?=$editorid?>_cmd_video" title="Video">Video</a>
<a id="<?=$editorid?>_cmd_flash" title="Flash">Flash</a>
<? } ?>
<a id="<?=$editorid?>_cmd_code" title="Insert Code">Insert Code</a>
<a id="<?=$editorid?>_cmd_quote" title="Insert Quote">Insert Quote</a>
</div>
...
var imgexts = '<?=$imgexts?>';
<? } ?>
var fontoptions = new Array("Trebuchet MS", "Tahoma", "Arial", "Impact", "Verdana", "Times New Roman");
...
<p><strong>TAG:</strong> (Use "," to seperate tags, Max Tag Number is <strong>5</strong>)</p>
<p><input type="text" name="tags" id="tags" class="txt" value="<?=$threadtags?>" tabindex="1" /><button name="addtags" type="button" onclick="relatekw();return false">+Search Tags</button> <span id="tagselect"></span></p>
</div>
</td>
<? } if($allowsetreadperm) { ?>
<td class="firstlist">
<p><strong>Read Permission:</strong></p>
<p><input type="text" name="readperm" value="<?=$thread['readperm']?>" class="txt" tabindex="1" /> zero or blank means unlimited</p>
</td>
<? } if($maxprice && !$special) { ?>
<td class="firstlist">
<p><strong>Price</strong>(<?=$extcredits[$creditstransextra['1']]['title']?>):</p>
<p><input type="text" name="price" value="<?=$thread['pricedisplay']?>" class="txt" tabindex="1" /> <?=$extcredits[$creditstransextra['1']]['unit']?> <span title="<code><strong>[free]</strong>message<strong>[/free]</strong></code> is available for free of charge message block"><strong>Highest</strong>: <?=$maxprice?> <?=$extcredits[$creditstransextra['1']]['unit']?>.</span>
</td>
<? } ?>
</tr></table>
</div>
...
<span class="lighttxt">
<? if($maxincperthread) { ?><img src="<?=IMGDIR?>/arrow_right.gif" />Highest author income per thread is <?=$maxincperthread?> <?=$extcredits[$creditstransextra['1']]['unit']?><? } if($maxchargespan) { ?><img src="<?=IMGDIR?>/arrow_right.gif" />longest charging time is <?=$maxchargespan?> hours<? if($action == 'edit' && $freechargehours) { ?>,This topic is also sold <?=$freechargehours?> Hours<? } } ?>
</span>
<? } ?>
<div class="columntype">
<h4>Post Option:</h4>
...
<p><input type="checkbox" name="htmlon" id="htmlon" value="1" <?=$htmloncheck?> /><label for="htmlon">Html Code</label></p>
<? } else { ?>
<p><input type="checkbox" name="htmlon" id="htmlon" value="0" <?=$htmloncheck?> disabled="disabled" /><label for="htmlon">Html Code</label></p>
<? } ?>
<p><input type="checkbox" id="allowimgcode" disabled<? if($forum['allowimgcode']) { ?> checked="checked"<? } ?> /><label for="allowimgcode">[img] Code</label></p>
</td>
<td>
<p><input type="checkbox" name="parseurloff" id="parseurloff" value="1" <?=$urloffcheck?> tabindex="1" /><label for="parseurloff">Disable Parse URL</label></p>
<p><input type="checkbox" name="smileyoff" id="smileyoff" value="1" <?=$smileyoffcheck?> tabindex="1" /><label for="smileyoff">Disable <a href="faq.php?action=faq&amp;id=5&amp;messageid=32" target="_blank">Smilies</a></label></p>
<p><input type="checkbox" name="bbcodeoff" id="bbcodeoff" value="1" <?=$codeoffcheck?> tabindex="1" /><label for="bbcodeoff">Disable <a href="faq.php?action=faq&amp;id=5&amp;messageid=18" target="_blank">Discuz! Code</a></label></p>
<? if($tagstatus && ($action == 'newthread' || $action == 'edit' && $isfirstpost)) { ?>
<p><input type="checkbox" name="tagoff" id="tagoff" <? if(!empty($tagoffcheck)) { ?>checked="checked"<? } ?> tabindex="1" /><label for="tagoff">Disable Parse Tag</label></p>
<? } ?>
</td>
<td>
<? if($discuz_uid) { if($action == 'newthread' || $action == 'edit') { ?>
<p><input type="checkbox" name="ordertype" id="ordertype" value="1" <?=$ordertypecheck?> tabindex="1" /><label for="ordertype">Desc view</label></p>
<? } if($action == 'newthread') { ?>
<p><input type="checkbox" name="attention_add" id="attention_add" checked="checked" value="1" /><label for="attention_add">Add to My attention, if there is a New Reply</label></p>
<? } elseif($action == 'reply' || $action == 'edit') { if($has_attention) { ?>
<p><input type="checkbox" name="attention_remove" id="attention_remove" value="1" /><label for="attention_remove">Remove notification of new reply</label></p>
<? } else { ?>
<p><input type="checkbox" name="attention_add" id="attention_add" value="1" /><label for="attention_add">Add to My attention, if there is a New Reply</label></p>
<? } } } ?>
<p><input type="checkbox" name="usesig" id="usesig" value="1" <? if(!$maxsigsize) { ?>disabled <? } else { ?><?=$usesigcheck?> <? } ?>tabindex="1" /><label for="usesig">Show Signature</label></p>
<? if($action != 'edit') { if($allowanonymous) { ?><p><input type="checkbox" name="isanonymous" id="isanonymous" value="1" tabindex="1" /><label for="isanonymous">Post Anonymous</label></p><? } } else { if($allowanonymous || (!$allowanonymous && $orig['anonymous'])) { ?><p><input type="checkbox" name="isanonymous" id="isanonymous" value="1" <? if($orig['anonymous']) { ?>checked="checked"<? } ?> tabindex="1" /><label for="isanonymous">Post Anonymous</label></p><? } } ?>
</td>
<td>
...
<p><input type="checkbox" name="sticktopic" id="sticktopic" value="1" <?=$stickcheck?> tabindex="1" /><label for="sticktopic">Stick</label></p>
<p><input type="checkbox" name="addtodigest" id="addtodigest" value="1" <?=$digestcheck?> tabindex="1" /><label for="addtodigest">Digest</label></p>
<? } } elseif($action == 'edit' && $auditstatuson) { ?>
<p><input type="checkbox" name="audit" id="audit" value="1"><label for="audit">Pass Audit</label></p>
<? } if($uchome['addfeed'] && $ucappopen['UCHOME'] && $forum['allowfeed']) { ?>
<p><input type="checkbox" name="addfeed" id="addfeed" value="1" <?=$addfeedcheck?>><label for="addfeed">Add Feed</label></p>
...
<a href="javascript:;" id="custominfo" class="right"><img src="<?=IMGDIR?>/info.gif" alt="FAQ" /></a>
<? } ?>
</span>
...
<? if($action == 'newthread') { if($special == 0) { ?>Post Thread
<? } elseif($special == 1) { ?>Post Poll
<? } elseif($special == 2) { ?>Post Trade
<? } elseif($special == 3) { ?>Post Reward
<? } elseif($special == 4) { ?>Post Activity
<? } elseif($special == 5) { ?>Post Debate
<? } elseif($special == 127) { if($buttontext) { ?><?=$buttontext?><? } else { ?>Post Thread<? } } } elseif($action == 'reply' && !empty($addtrade)) { ?>Add
<? } elseif($action == 'reply') { ?>Join/Reply Thread
<? } elseif($action == 'edit') { ?>Edit Thread
<? } ?>
</button>
...
<ul unselectable="on">
<li id="<?=$editorid?>_cmd_bold" onclick="discuzcode('bold')" unselectable="on">Bold</li>
<li id="<?=$editorid?>_cmd_italic" onclick="discuzcode('italic')" unselectable="on">Italic</li>
<li id="<?=$editorid?>_cmd_underline" onclick="discuzcode('underline')" unselectable="on">Underline</li>
<li id="<?=$editorid?>_cmd_strikethrough" onclick="discuzcode('strikethrough')" unselectable="on">Delete Line</li>
<li id="<?=$editorid?>_cmd_inserthorizontalrule" onclick="discuzcode('inserthorizontalrule')" unselectable="on">Separator</li>
</ul>
</div><? $fontoptions = array("Trebuchet MS", "Tahoma", "Arial", "Impact", "Verdana", "Times New Roman") ?><div class="popupmenu_popup fontname_menu" id="<?=$editorid?>_cmd_fontname_menu" style="display: none">
...
<ul unselectable="on">
<li><a id="<?=$editorid?>_cmd_justifycenter" onclick="discuzcode('justifycenter')" unselectable="on">Center</a></li>
<li><a id="<?=$editorid?>_cmd_justifyleft" onclick="discuzcode('justifyleft')" unselectable="on">Left</a></li>
<li><a id="<?=$editorid?>_cmd_justifyright" onclick="discuzcode('justifyright')" unselectable="on">Right</a></li>
<li><a id="<?=$editorid?>_cmd_autotypeset" onclick="discuzcode('autotypeset')" unselectable="on">Automatic Layout</a></li>
</ul>
</div>

<div class="popupmenu_popup simple_menu" id="<?=$editorid?>_cmd_list_menu" style="display: none">
<ul unselectable="on">
<li id="<?=$editorid?>_cmd_insertorderedlist" onclick="discuzcode('insertorderedlist')" unselectable="on">Order List</li>
<li id="<?=$editorid?>_cmd_insertunorderedlist" onclick="discuzcode('insertunorderedlist')" unselectable="on">Unorder List</li>
</ul>
</div>
...
<div class="float_ctrl"><span><a href="javascript:;" class="float_close" onclick="hideMenu()">Close</a></span></div>
<ul class="imguptype" id="<?=$editorid?>_cmd_image_ctrl">
<li><a href="javascript:;" hidefocus="true" class="current" id="<?=$editorid?>_btn_www" onclick="switchImagebutton('www');">Internet Picture</a></li>
<? if($allowpostimg) { ?>
<li><a href="javascript:;" hidefocus="true" id="<?=$editorid?>_btn_imgattachlist" onclick="switchImagebutton('imgattachlist');">Attach Image</a></li>
<? if($swfupload != 1 && $allowuploadnum) { ?><li><a href="javascript:;" hidefocus="true" id="<?=$editorid?>_btn_local" onclick="switchImagebutton('local');">Normal Upload</a></li><? } if($swfupload != 0 && $allowuploadnum) { ?><li><a href="javascript:;" hidefocus="true" id="<?=$editorid?>_btn_multi" onclick="switchImagebutton('multi');">Batch Upload</a></li><? } } ?>
</ul>
<div class="popupmenu_option" unselectable="on" id="<?=$editorid?>_www">
<table cellpadding="0" cellspacing="0" width="100%">
<tr>
<th width="74%">Insert Picture URL</th>
<th width="13%">Width(pixels)</th>
<th width="13%">Height(pixels)</th>
</tr>
<tr>
...
<tr>
<td colspan="3" align="center"><input type="button" id="<?=$editorid?>_cmd_image_submit" value="Submit" /> &nbsp; <input onclick="hideMenu();" value="Cancel" type="button" /></td>
</tr>
</table>
</div>
...
<p id="imguploadbtn"><input type="button" value="Upload" onclick="uploadAttach(0, 0, 'img')" /> &nbsp; <input type="button" value="Cancel" onclick="hideMenu();" /></p>
<p id="imguploading" style="display: none;"><img src="<?=IMGDIR?>/uploading.gif" style="vertical-align: middle;" /> Uploading, please wait, you can<a href="javascript:;" onclick="hideMenu()">close this small window temporarily</a>.You will be notified after the upload is complete.</p>
</div>
<div class="notice uploadinfo">
Size: <strong><? if($maxattachsize) { ?>lower than <?=$maxattachsize_mb?> <? } else { ?>Size unlimited<? } ?></strong>&nbsp;
<? if($maxattachnum || $maxsizeperday) { ?>
Upload Limit: <? if($maxattachnum) { ?><strong><?=$allowuploadnum?></strong> Files&nbsp;<? } if($maxsizeperday) { ?><strong><?=$allowuploadsize?></strong>&nbsp;<? } } if($imgexts) { ?>
<br />Allowed File: <strong><?=$imgexts?></strong>&nbsp;
<? } if($creditstring) { ?>
<br /><a href="faq.php?action=credits&amp;fid=<?=$fid?>" target="_blank" title="Credits Policy">Upload one of your &nbsp;<?=$creditstring?></a>
<? } ?>
</div>
</div>
<? } if($swfupload != 0) { ?>
...
<p class="notice">This post does not have images,<? if($allowuploadnum) { ?>, <a href="javascript:;" onclick="switchImagebutton('<? if($swfupload != 0) { ?>multi<? } else { ?>local<? } ?>');">Click here</a>Upload<? } ?></p>
<? } ?>
</div>
<div id="unusedimgattachlist">
<? if($imgattachs['unused']) { $imagelist = $imgattachs['unused']; ?><p>You have uploaded <?=$swfattachs?> Attachements<p><? include template('ajax_imagelist', '0', ''); } ?>
</div>
<? if($ucappopen['UCHOME']) { ?>
<p class="notice">Select a picture from my album: <span id="uch_albums"></span></p>
<div id="uch_photoes"></div>
<? } ?>
</div>
<p class="notice" id="imgattach_notice"<? if(!$imgattachs['used'] && !$imgattachs['unused']) { ?> style="display: none"<? } ?>>Click to insert</p>
</div>
<? } ?>
</div>
...
<li><a href="javascript:;" hidefocus="true" class="current" id="<?=$editorid?>_btn_attachlist" onclick="switchAttachbutton('attachlist');">Attachment List</a></li>
<? if($swfupload != 1 && $allowuploadnum) { ?><li><a href="javascript:;" hidefocus="true" id="<?=$editorid?>_btn_upload" onclick="switchAttachbutton('upload');">Common Upload</a></li><? } if($swfupload != 0 && $allowuploadnum) { ?><li><a href="javascript:;" hidefocus="true" id="<?=$editorid?>_btn_swfupload" onclick="switchAttachbutton('swfupload');">Batch Upload</a></li><? } ?>
</ul>
<? if($swfupload != 1) { ?>
...
<p id="uploadbtn"><input type="button" value="Upload" onclick="uploadAttach(0, 0)" /> &nbsp; <input type="button" value="Cancel" onclick="hideMenu();" /></p>
<p id="uploading" style="display: none;"><img src="<?=IMGDIR?>/uploading.gif" style="vertical-align: middle;" /> Uploading, please wait, you can<a href="javascript:;" onclick="hideMenu()">close this small window temporarily</a>.You will be notified after the upload is complete.</p>
</div>
<div class="notice uploadinfo">
Size: <strong><? if($maxattachsize) { ?>lower than <?=$maxattachsize_mb?> <? } else { ?>Size unlimited<? } ?></strong>&nbsp;
<? if($maxattachnum || $maxsizeperday) { ?>
Upload Limit: <? if($maxattachnum) { ?><strong><?=$allowuploadnum?></strong> Files&nbsp;<? } if($maxsizeperday) { ?><strong><?=$allowuploadsize?></strong>&nbsp;<? } } if($attachextensions) { ?>
<br />Allowed File: <strong><?=$attachextensions?></strong>&nbsp;
<? } if($creditstring) { ?>
<br /><a href="faq.php?action=credits&amp;fid=<?=$fid?>" target="_blank" title="Credits Policy">Upload one of your &nbsp;<?=$creditstring?></a>
<? } ?>
</div>
...
<td class="attachname">FileName</td>
<td class="attachdesc">Description</td>
<? if($allowsetattachperm) { ?><td class="attachview" style="width: 52px;">Read Permission</td><? } if($maxprice) { ?><td class="attachpr"><?=$extcredits[$creditstransextra['1']]['title']?></td><? } ?>
...
<p class="notice">This post does not have attachment,<? if($allowuploadnum) { ?>, <a href="javascript:;" onclick="switchAttachbutton('<? if($swfupload != 0) { ?>swfupload<? } else { ?>upload<? } ?>');">Click here</a>Upload<? } ?></p>
<? } ?>
</div>
<div id="unusedattachlist">
<? if($attachs['unused']) { $attachlist = $attachs['unused']; ?><table cellpadding="0" cellspacing="0" width="100%"><tr><td class="attachnum"></td><td>You have uploaded <?=$swfattachs?> Attachements</td></tr></table><? include template('ajax_attachlist', '0', ''); } ?>
</div>
</div>
<p class="notice" id="attach_notice"<? if(!$attachs['used'] && !$attachs['unused']) { ?> style="display: none"<? } ?>>Insert Attach Image</p>
<? if($maxprice && $maxincperthread) { ?><p class="notice">Highest Attachment incme per thread is  <?=$maxincperthread?> <?=$extcredits[$creditstransextra['1']]['unit']?>&nbsp;&nbsp;&nbsp;&nbsp;</p><? } ?>
</div>
</div>
<? } ?>
...
var msg = 'You have uploaded <?=$swfattachs?> Attachements<div style="<? if(count($attachs['unused']) + count($imgattachs['unused']) > 10) { ?>height:10em;<? } ?>overflow-y:auto;overflow-x:hidden">'<? if(is_array($attachs['unused'])) { foreach($attachs['unused'] as $attach) { ?>+ '&nbsp;&nbsp;&nbsp;&nbsp;<span title="<?=$attach['filenametitle']?> <?=$attach['dateline']?>"><?=$attach['filename']?></span><br />'<? } } if(is_array($imgattachs['unused'])) { foreach($imgattachs['unused'] as $attach) { ?>+ '&nbsp;&nbsp;&nbsp;&nbsp;<span title="<?=$attach['filenametitle']?> <?=$attach['dateline']?>"><?=$attach['filename']?></span><br />'<? } } ?>+ '</div><div class="alert_btn"><input type="button" id="fwin_dialog_submit" onclick="hideMenu(\'fwin_dialog\', \'dialog\')" value="&nbsp;Use&nbsp;" /> <input type="button" onclick="$(\'unusedimgattachlist\').parentNode.removeChild($(\'unusedimgattachlist\'));$(\'unusedattachlist\').parentNode.removeChild($(\'unusedattachlist\'));hideMenu(\'fwin_dialog\', \'dialog\');ATTACHNUM[\'imageunused\']=0;ATTACHNUM[\'attachunused\']=0;updateattachnum(\'image\');updateattachnum(\'attach\');" value="&nbsp;Ignore&nbsp;" /></div>';
