...
function setCopy(text, msg){
	if(BROWSER.ie) {
		...
		var msg = '<div style="text-decoration:underline;">Copy to Clipboard</div>' +
	...
	if(!$(menuid)) {
		...
		if(ctrlid) {
			...
			msg = '<table cellspacing="0" cellpadding="0" class="popupcredit"><tr><td class="pc_l">&nbsp;</td><td class="pc_c"><div class="pc_inner">' + msg +
				(negligible ? '<a class="pc_btn" href="javascript:;" onclick="display(\'ntcwin\');setcookie(\'discuz_creditnoticedisable\', 1, 31536000);" title="Not show again"><img src="' + IMGDIR + '/popupcredit_btn.gif" alt="Not show again" /></a>' : '') +
...
function showDialog(msg, mode, t, func, cover) {
	...
	if(mode == 'info') {
		...
		s += '<div class="alert_btn"><input type="button" id="fwin_dialog_submit" value="Submit" />';
		s += mode == 'confirm' ? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" value="Cancel" />' : '';
		s += '</div></div>';
	}
...
function showselect(obj, inpid, t, rettype) {
	if(!obj.id) {
		...
		if(!t) {
			...
			s += showselect_row(inpid, '3 month3', 90, 0, rettype);
			s += showselect_row(inpid, 'Customize', -2);
		} else {
			if($(t)) {
				...
				s += showselect_row(inpid, 'Customize', -1);
			} else {
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">Unlimited</a><br />';
				...
				s += showselect_row(inpid, '3 months', 90, 1, rettype);
				s += showselect_row(inpid, '6 months', 182, 1, rettype);
...
function removeindexheats() {
	return confirm('Remove this thread from hot threads?');
}
...
function smilies_switch(id, smcols, type, page, seditorkey) {
	...
	if(smilies_array[type].length > 2) {
		...
		smiliespage = '<div class="pags_act"><a href="javascript:;" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', ' + type + ', ' + prevpage + ', \'' + seditorkey + '\');doane(event);">Pre</a>' +
...
function showimmestatus(imme) {
	var lang = {'Online':'MSN Online','Busy':'MSN Busy','Away':'MSN Away','Offline':'MSN Offilne'};
