<?php
//Discuz! cache file, DO NOT modify me!
//Created: Sep 22, 2013, 15:16
//Identify: ea56b92e7b3256829b2366cdc01e701e

$_DCACHE['forums'] = array (
  1 => 
  array (
    'fid' => '1',
    'type' => 'group',
    'name' => 'Discuz!',
    'fup' => '0',
    'viewperm' => '',
    'orderby' => 'lastpost',
    'ascdesc' => 'DESC',
    'status' => '1',
    'extra' => 
    array (
    ),
  ),
  2 => 
  array (
    'fid' => '2',
    'type' => 'forum',
    'name' => 'Default Forum',
    'fup' => '1',
    'viewperm' => '',
    'orderby' => 'lastpost',
    'ascdesc' => 'DESC',
    'users' => NULL,
    'status' => '1',
    'extra' => 
    array (
    ),
  ),
);

$_DCACHE['stamps'] = array (
  0 => 
  array (
    'url' => '001.gif',
    'text' => '精华',
  ),
  1 => 
  array (
    'url' => '002.gif',
    'text' => '热帖',
  ),
  2 => 
  array (
    'url' => '003.gif',
    'text' => '美图',
  ),
  3 => 
  array (
    'url' => '004.gif',
    'text' => '优秀',
  ),
  4 => 
  array (
    'url' => '005.gif',
    'text' => '置顶',
  ),
  5 => 
  array (
    'url' => '006.gif',
    'text' => '推荐',
  ),
  6 => 
  array (
    'url' => '007.gif',
    'text' => '原创',
  ),
  7 => 
  array (
    'url' => '008.gif',
    'text' => '版主推荐',
  ),
  8 => 
  array (
    'url' => '009.gif',
    'text' => '爆料',
  ),
);

$_DCACHE['ranks'] = array (
);

$_DCACHE['usergroups'] = array (
  1 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Administrator',
    'stars' => '9',
    'groupavatar' => '',
    'readaccess' => '200',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  18 => 
  array (
    'type' => 'special',
    'grouptitle' => 'Information Monitors',
    'stars' => '9',
    'groupavatar' => '',
    'readaccess' => '200',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  17 => 
  array (
    'type' => 'special',
    'grouptitle' => 'Website Editor',
    'stars' => '8',
    'groupavatar' => '',
    'readaccess' => '150',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  16 => 
  array (
    'type' => 'special',
    'grouptitle' => 'Practise Moderator',
    'stars' => '7',
    'groupavatar' => '',
    'readaccess' => '100',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  19 => 
  array (
    'type' => 'special',
    'grouptitle' => 'Auditor',
    'stars' => '7',
    'groupavatar' => '',
    'readaccess' => '100',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  9 => 
  array (
    'type' => 'member',
    'grouptitle' => 'Beggar',
    'creditshigher' => '-9999999',
    'creditslower' => '0',
    'stars' => '0',
    'groupavatar' => '',
    'readaccess' => '0',
    'allowcusbbcode' => '0',
    'allowgetattach' => '0',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  8 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Awaiting Verification',
    'stars' => '0',
    'groupavatar' => '',
    'readaccess' => '0',
    'allowcusbbcode' => '0',
    'allowgetattach' => '0',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  7 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Guests',
    'stars' => '0',
    'groupavatar' => '',
    'readaccess' => '1',
    'allowcusbbcode' => '0',
    'allowgetattach' => '0',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  6 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Banned IP',
    'stars' => '0',
    'groupavatar' => '',
    'readaccess' => '0',
    'allowcusbbcode' => '0',
    'allowgetattach' => '0',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  5 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Banned',
    'stars' => '0',
    'groupavatar' => '',
    'readaccess' => '0',
    'allowcusbbcode' => '0',
    'allowgetattach' => '0',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  2 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Super Moderator',
    'stars' => '8',
    'groupavatar' => '',
    'readaccess' => '150',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  3 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Moderator',
    'stars' => '7',
    'groupavatar' => '',
    'readaccess' => '100',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  4 => 
  array (
    'type' => 'system',
    'grouptitle' => 'Banned to Post',
    'stars' => '0',
    'groupavatar' => '',
    'readaccess' => '0',
    'allowcusbbcode' => '0',
    'allowgetattach' => '0',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  10 => 
  array (
    'type' => 'member',
    'grouptitle' => 'Newbie',
    'creditshigher' => '0',
    'creditslower' => '50',
    'stars' => '1',
    'groupavatar' => '',
    'readaccess' => '10',
    'allowcusbbcode' => '0',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  11 => 
  array (
    'type' => 'member',
    'grouptitle' => 'Member',
    'creditshigher' => '50',
    'creditslower' => '200',
    'stars' => '2',
    'groupavatar' => '',
    'readaccess' => '20',
    'allowcusbbcode' => '0',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  12 => 
  array (
    'type' => 'member',
    'grouptitle' => 'Conqueror',
    'creditshigher' => '200',
    'creditslower' => '500',
    'stars' => '3',
    'groupavatar' => '',
    'readaccess' => '30',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  13 => 
  array (
    'type' => 'member',
    'grouptitle' => 'Lord',
    'creditshigher' => '500',
    'creditslower' => '1000',
    'stars' => '4',
    'groupavatar' => '',
    'readaccess' => '50',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  14 => 
  array (
    'type' => 'member',
    'grouptitle' => 'King',
    'creditshigher' => '1000',
    'creditslower' => '3000',
    'stars' => '6',
    'groupavatar' => '',
    'readaccess' => '70',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
  15 => 
  array (
    'type' => 'member',
    'grouptitle' => 'Forum Legend',
    'creditshigher' => '3000',
    'creditslower' => '9999999',
    'stars' => '8',
    'groupavatar' => '',
    'readaccess' => '90',
    'allowcusbbcode' => '1',
    'allowgetattach' => '1',
    'edittimelimit' => '0',
    'userstatusby' => 1,
  ),
);

$_DCACHE['smilies'] = array (
  'searcharray' => 
  array (
    13 => '/\\:loveliness\\:/',
    23 => '/\\:handshake/',
    20 => '/\\:victory\\:/',
    37 => '/\\{\\:2_37\\:\\}/',
    46 => '/\\{\\:3_46\\:\\}/',
    55 => '/\\{\\:3_55\\:\\}/',
    32 => '/\\{\\:2_32\\:\\}/',
    64 => '/\\{\\:3_64\\:\\}/',
    41 => '/\\{\\:3_41\\:\\}/',
    18 => '/\\:sleepy\\:/',
    50 => '/\\{\\:3_50\\:\\}/',
    27 => '/\\{\\:2_27\\:\\}/',
    59 => '/\\{\\:3_59\\:\\}/',
    36 => '/\\{\\:2_36\\:\\}/',
    45 => '/\\{\\:3_45\\:\\}/',
    54 => '/\\{\\:3_54\\:\\}/',
    31 => '/\\{\\:2_31\\:\\}/',
    63 => '/\\{\\:3_63\\:\\}/',
    40 => '/\\{\\:2_40\\:\\}/',
    17 => '/\\:shutup\\:/',
    49 => '/\\{\\:3_49\\:\\}/',
    26 => '/\\{\\:2_26\\:\\}/',
    58 => '/\\{\\:3_58\\:\\}/',
    35 => '/\\{\\:2_35\\:\\}/',
    44 => '/\\{\\:3_44\\:\\}/',
    53 => '/\\{\\:3_53\\:\\}/',
    30 => '/\\{\\:2_30\\:\\}/',
    62 => '/\\{\\:3_62\\:\\}/',
    39 => '/\\{\\:2_39\\:\\}/',
    48 => '/\\{\\:3_48\\:\\}/',
    25 => '/\\{\\:2_25\\:\\}/',
    57 => '/\\{\\:3_57\\:\\}/',
    34 => '/\\{\\:2_34\\:\\}/',
    43 => '/\\{\\:3_43\\:\\}/',
    52 => '/\\{\\:3_52\\:\\}/',
    29 => '/\\{\\:2_29\\:\\}/',
    61 => '/\\{\\:3_61\\:\\}/',
    38 => '/\\{\\:2_38\\:\\}/',
    47 => '/\\{\\:3_47\\:\\}/',
    56 => '/\\{\\:3_56\\:\\}/',
    33 => '/\\{\\:2_33\\:\\}/',
    42 => '/\\{\\:3_42\\:\\}/',
    51 => '/\\{\\:3_51\\:\\}/',
    28 => '/\\{\\:2_28\\:\\}/',
    60 => '/\\{\\:3_60\\:\\}/',
    16 => '/\\:dizzy\\:/',
    15 => '/\\:curse\\:/',
    14 => '/\\:funk\\:/',
    22 => '/\\:kiss\\:/',
    21 => '/\\:time\\:/',
    24 => '/\\:call\\:/',
    19 => '/\\:hug\\:/',
    12 => '/\\:lol/',
    4 => '/\\:\'\\(/',
    5 => '/\\:@/',
    9 => '/;P/',
    8 => '/\\:\\$/',
    3 => '/\\:D/',
    7 => '/\\:P/',
    2 => '/\\:\\(/',
    11 => '/\\:Q/',
    6 => '/\\:o/',
    1 => '/\\:\\)/',
    10 => '/\\:L/',
  ),
  'replacearray' => 
  array (
    13 => 'loveliness.gif',
    23 => 'handshake.gif',
    20 => 'victory.gif',
    37 => '13.gif',
    46 => '06.gif',
    55 => '15.gif',
    32 => '08.gif',
    64 => '24.gif',
    41 => '01.gif',
    18 => 'sleepy.gif',
    50 => '10.gif',
    27 => '03.gif',
    59 => '19.gif',
    36 => '12.gif',
    45 => '05.gif',
    54 => '14.gif',
    31 => '07.gif',
    63 => '23.gif',
    40 => '16.gif',
    17 => 'shutup.gif',
    49 => '09.gif',
    26 => '02.gif',
    58 => '18.gif',
    35 => '11.gif',
    44 => '04.gif',
    53 => '13.gif',
    30 => '06.gif',
    62 => '22.gif',
    39 => '15.gif',
    48 => '08.gif',
    25 => '01.gif',
    57 => '17.gif',
    34 => '10.gif',
    43 => '03.gif',
    52 => '12.gif',
    29 => '05.gif',
    61 => '21.gif',
    38 => '14.gif',
    47 => '07.gif',
    56 => '16.gif',
    33 => '09.gif',
    42 => '02.gif',
    51 => '11.gif',
    28 => '04.gif',
    60 => '20.gif',
    16 => 'dizzy.gif',
    15 => 'curse.gif',
    14 => 'funk.gif',
    22 => 'kiss.gif',
    21 => 'time.gif',
    24 => 'call.gif',
    19 => 'hug.gif',
    12 => 'lol.gif',
    4 => 'cry.gif',
    5 => 'huffy.gif',
    9 => 'titter.gif',
    8 => 'shy.gif',
    3 => 'biggrin.gif',
    7 => 'tongue.gif',
    2 => 'sad.gif',
    11 => 'mad.gif',
    6 => 'shocked.gif',
    1 => 'smile.gif',
    10 => 'sweat.gif',
  ),
  'typearray' => 
  array (
    13 => '1',
    23 => '1',
    20 => '1',
    37 => '2',
    46 => '3',
    55 => '3',
    32 => '2',
    64 => '3',
    41 => '3',
    18 => '1',
    50 => '3',
    27 => '2',
    59 => '3',
    36 => '2',
    45 => '3',
    54 => '3',
    31 => '2',
    63 => '3',
    40 => '2',
    17 => '1',
    49 => '3',
    26 => '2',
    58 => '3',
    35 => '2',
    44 => '3',
    53 => '3',
    30 => '2',
    62 => '3',
    39 => '2',
    48 => '3',
    25 => '2',
    57 => '3',
    34 => '2',
    43 => '3',
    52 => '3',
    29 => '2',
    61 => '3',
    38 => '2',
    47 => '3',
    56 => '3',
    33 => '2',
    42 => '3',
    51 => '3',
    28 => '2',
    60 => '3',
    16 => '1',
    15 => '1',
    14 => '1',
    22 => '1',
    21 => '1',
    24 => '1',
    19 => '1',
    12 => '1',
    4 => '1',
    5 => '1',
    9 => '1',
    8 => '1',
    3 => '1',
    7 => '1',
    2 => '1',
    11 => '1',
    6 => '1',
    1 => '1',
    10 => '1',
  ),
);

$_DCACHE['smileytypes'] = array (
  1 => 
  array (
    'name' => 'Default',
    'directory' => 'default',
  ),
  2 => 
  array (
    'name' => 'Monkey',
    'directory' => 'coolmonkey',
  ),
  3 => 
  array (
    'name' => 'GrapMan',
    'directory' => 'grapeman',
  ),
);

$_DCACHE['bbcodes'] = array (
  'searcharray' => 
  array (
    0 => '/\\[qq]([^"\\[]+?)\\[\\/qq\\]/is',
  ),
  'replacearray' => 
  array (
    0 => '<a href="http://wpa.qq.com/msgrd?V=1&Uin=\\1&amp;Site=[Discuz!]&amp;Menu=yes" target="_blank"><img src="http://wpa.qq.com/pa?p=1:\\1:1" border="0"></a>',
  ),
);

$_DCACHE['advs'] = array (
);

$_DCACHE['tags'] = array (
  1 => '[\'Windows\',\'Migration\']',
  0 => '[\'Windows\',\'Migration\']',
  2 => '2',
);

$_DCACHE['custominfo'] = array (
  'customauthorinfo' => 
  array (
    2 => '<dt>UID</dt><dd>$post[uid]&nbsp;</dd><dt>Posts</dt><dd>$post[posts]&nbsp;</dd><dt>Digests</dt><dd>$post[digestposts]&nbsp;</dd><dt>Credits</dt><dd>$post[credits]&nbsp;</dd><dt>Read Permission</dt><dd>$post[readaccess]&nbsp;</dd>".($post[location] ? "<dt>Location</dt><dd>$post[location]&nbsp;</dd>" : "")."<dt>Online</dt><dd>$post[oltime] Hours&nbsp;</dd><dt>Joined</dt><dd>$post[regdate]&nbsp;</dd><dt>Last Visit</dt><dd>$post[lastdate]&nbsp;</dd>',
    1 => NULL,
  ),
  'fieldsadd' => '',
  'profilefields' => 
  array (
  ),
  'postno' => 
  array (
    0 => '<sup>#</sup>',
    1 => '',
  ),
);

$_DCACHE['groupicon'] = array (
  1 => 'images/common/online_admin.gif',
  2 => 'images/common/online_supermod.gif',
  3 => 'images/common/online_moderator.gif',
  0 => 'images/common/online_member.gif',
);

$_DCACHE['focus'] = array (
  'title' => NULL,
  'data' => 
  array (
  ),
);

?>