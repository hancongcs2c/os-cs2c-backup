...
<h3 class="float_ctrl">
<em id="return_mods">Select <?=$modpostsnum?> Posts</em>
<span>
<a href="javascript:;" class="float_close" onclick="hideWindow('mods')" title="Close">Close</a>
</span>
</h3>
...
<td>
<label onclick="switchitemcp('itemcp_digest')" class="labeltxt">Add Digest</label>
<div class="detailopt">
<select name="digestlevel">
<option value="0">Remove</option>
<option value="1" <?=$digestcheck['1']?>>Digest 1</option>
<? if($allowdigestthread >= 2) { ?>
<option value="2" <?=$digestcheck['2']?>>Digest 2</option>
<? if($allowdigestthread == 3) { ?>
<option value="3" <?=$digestcheck['3']?>>Digest 3</option>
<? } } ?>
</select>
</div>
</td>
...
<td><? $colorarray = array(1=>'#EE1B2E', 2=>'#EE5023', 3=>'#996600', 4=>'#3C9D40', 5=>'#2897C5', 6=>'#2B65B7', 7=>'#8F2A90', 8=>'#EC1282'); ?><label onclick="switchitemcp('itemcp_highlight')" class="labeltxt">Highlight</label>
<div class="detailopt">
...
<a href="javascript:;" id="highlight_op_1" onclick="switchhl(this, 1)" class="detailopt_bold<? if($stylecheck['1']) { ?> current<? } ?>" style="text-indent:0;text-decoration:none;font-weight:700;" title="Bold">B</a>
<a href="javascript:;" id="highlight_op_2" onclick="switchhl(this, 2)" class="detailopt_italic<? if($stylecheck['2']) { ?> current<? } ?>" style="text-indent:0;text-decoration:none;font-style:italic;" title="Italic">I</a>
<a href="javascript:;" id="highlight_op_3" onclick="switchhl(this, 3)" class="detailopt_underline<? if($stylecheck['3']) { ?> current<? } ?>" style="text-indent:0;text-decoration:underline;" title="Underline">U</a>
</div>
</td>
</tr>
<tr class="detailopt">
...
<div class="detailopt">
<label><input class="radio" type="radio" name="isrecommend" value="1" checked="checked" /> Recommend</label>
<label><input class="radio" type="radio" name="isrecommend" value="0" /> Unrecommend</label>
</div>
</td>
</tr>
<tr class="detailopt">
...
<td></td>
<td>
<label class="labeltxt">Image</label>
<select name="selectattach" onchange="updateimginfo(this.value)" style="width: 125px">
<option value="">No Image</option><? if(is_array($imgattach)) { foreach($imgattach as $imginfo) { ?><option value="<?=$imginfo['aid']?>"<? if($selectattach == $imginfo['aid']) { ?> selected="selected"<? } ?>><?=$imginfo['filename']?></option><? } } ?></select>
</td>
</tr>
<tr class="detailopt">
<td></td>
...
Target Forum: <select id="moveto" name="moveto" onchange="if(this.value) {$('moveext').style.display='';} else {$('moveext').style.display='none';}">
<?=$forumselect?>
</select>
</p>
<p class="tah_body">
<ul class="inlinelist" id="moveext" style="display:none;margin:5px 0;">
<li class="wide"><label><input class="radio" type="radio" name="type" value="normal" checked="checked" /> Move</label></li>
<li class="wide"><label><input class="radio" type="radio" name="type" value="redirect" /> Hold Move Link</label></li>
</ul>
</p>
<? } else { if($typeselect) { ?>
<input type="hidden" name="operations[]" value="type" />
<p>Types: <?=$typeselect?></p>
<? } else { ?>
No category in Current Forum, Please contact administrator<? $hiddensubmit = true; } } ?>
</div>
<? } elseif($optgroup == 3) { ?>
<div class="topicadminlow">
<ul class="inlinelist">
...
<p>Confirm to <strong>Delete</strong> Thread?</p>
<? } else { ?>
<p>You have no permission to delete this thread</p>
<? } } elseif($operation == 'down' || $operation='bump') { ?>
<li class="wide"><label><input class="radio" type="radio" name="operations[]" value="bump" checked="checked"/> Bump Thread</label></li>
<li class="wide"><label><input class="radio" type="radio" name="operations[]" value="down" /> Down Thread</label></li>
<? } ?>
</ul>
</div>
<? } elseif($optgroup == 4) { ?>
...
<ul class="inlinelist">
<li class="wide"><label><input class="radio" type="radio" name="operations[]" value="open" <?=$closecheck['0']?> />Unlock</label></li>
<li class="wide"><label><input class="radio" type="radio" name="operations[]" value="close" <?=$closecheck['1']?> />Lock</label></li>
</ul>
</td>
</tr>
</table>
...
<p>
<input name="modsubmit" type="submit" value="Confirmed" />
<input type="checkbox" name="sendreasonpm" id="sendreasonpm" class="checkbox"<? if($reasonpm == 2 || $reasonpm == 3) { ?> checked="checked" disabled="disabled"<? } ?> /> <label for="sendreasonpm" />Send PM to Author</label>
</p>
<? } ?>
</div>
</div>
