...
<h2>
<? if($discuz_uid) { ?>
<span style="font-weight: bold;"><a href="space.php?uid=<?=$discuz_uid?>" target="_blank"><?=$discuz_userss?></a>&nbsp;</span> <a href="###" onClick="parent.main.location = 'logging.php?action=logout&formhash=<?=FORMHASH?>&referer=' + parent.main.location">[Logout]</a>
<? } else { ?>
<strong>Guest:&nbsp;</strong><a href="<?=$regname?>" target="main" onClick="this.href += '?referer='+ escape(parent.main.location)">[<?=$reglinkname?>]</a> <a href="logging.php?action=login" target="main" onClick="this.href += '&referer='+ escape(parent.main.location)">[Login]</a>
<? } ?>
</h2>

<script type="text/javascript">
var tree = new dzTree('tree');
tree.addNode(0, -1, 'HOME', '<?=$indexname?>', 'main', true);
tree.addNode(99999999, 0, 'View New Thread', 'search.php?srchfrom=87000&searchsubmit=yes', 'main', true);<? if(is_array($forumlist)) { foreach($forumlist as $forumdata) { if($forumdata['type'] == 'group') { if($haschild[$forumdata['fid']]) { ?>
...
<h3>Active Users: <a href="member.php?action=online" target="main"><strong id="onlinenum"></strong>&nbsp;Users</a></h3>
</div>
<script type="text/javascript">
