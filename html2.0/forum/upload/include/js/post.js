...
var STATUSMSG = {'-1' : 'Server Error', '0' : 'Uploaded successfully', '1' : 'Not support this extensions', '2' : 'Attachment Size is 0', '3' : 'Attachment size exceeds the limit', '4' : 'Not support this extensions', '5' : 'Attachment size exceeds the limit', '6' : 'Total attachments size exceeds the limit', '7' : 'Image attachment is illegal', '8' : 'Attachment can not be saved', '9' : 'There is no legitimate files uploaded', '10' : 'Illegal Operation'};
...
function checklength(theform) {
	...
	showDialog('Current Length: ' + mb_strlen(message) + ' characters，' + (postmaxchars != 0 ? 'System Limit: ' + postminchars + ' to ' + postmaxchars + ' characters.' : ''), 'notice', 'Length Check');
}
...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && message == "") {
		showDialog('Please input title or content.');
		return false;
	} else if(mb_strlen(theform.subject.value) > 80) {
		showDialog('Your title is longer than 80 characters.');
		return false;
	}
	if(tradepost) {
		if(theform.item_name.value == '') {
			showDialog('Sorry, please input goods name.');
			return false;
		} else if(theform.item_price.value == '') {
			showDialog('Sorry, please input current price.');
			return false;
		} else if(!parseInt(theform.item_price.value)) {
			showDialog('Sorry, the price should be numbers.');
			return false;
			...
			showDialog('Sorry, goods amount should be numbers.');
	...
	if(in_array($('postsubmit').name, ['topicsubmit', 'editsubmit'])) {
		if(theform.typeid && (theform.typeid.options && theform.typeid.options[theform.typeid.selectedIndex].value == 0) && typerequired) {
			showDialog('Please choose related category.');
			return false;
		}
		if(special == 3 && isfirstpost) {
			if(theform.rewardprice.value == "") {
				showDialog('Sorry, please input reward credit.');
				return false;
			}
		} else if(special == 4 && isfirstpost) {
			if(theform.activityclass.value == "") {
				showDialog('Sorry, please input activity type.');
				return false;
			} else if($('starttimefrom_0').value == "" && $('starttimefrom_1').value == "") {
				showDialog('Sorry, please input activity start time.');
				return false;
			} else if(theform.activityplace.value == "") {
				showDialog('Sorry, please input activity place.');
				return false;
			}
		}
	}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showDialog('The length of your posts do not meet the requirements.\n\nCurrent Length: ' + mb_strlen(message) + ' characters\nSystem Limit: ' + postminchars + ' to ' + postmaxchars + ' characters');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('Some attachments is waiting to upload, ignore?')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('Some attachments is uploading, please wait a moment, the post will be published automatically...', 'notice');
		AUTOPOST = 1;
		return false;
	}
...
function loadData(quiet) {
	...
	if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
		if(!quiet) {
			showDialog('No data to restore!');
		}
		return;
	}

	if(!quiet && !confirm('This operation will replace the content of current post, continue?')) {
		return;
	}
...
function setEditorEvents() {
	var floatPic = function(e) {
		...
		if(!menu) {
			...
			menu.innerHTML = '<div class="popupmenu_option" unselectable="on"><ul unselectable="on"><li id="' + menuid + '_left" unselectable="on">Image Left</li><li id="' + menuid + '_right" unselectable="on">Image Right</li></ul></div>';
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['simple', 'paragraph', 'list', 'smilies', 'createlink', 'quote', 'code', 'free', 'hide', 'audio', 'video', 'flash', 'attach', 'image']) || cmd == 'table' && wysiwyg || in_array(cmd, ['fontname', 'fontsize', 'forecolor']) && !arg) {
		...
		if(txt = getSel()) {
			...
			while(listvalue = prompt('Please input an item.\r\nKeep blank or click cancel to finish this list.', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'createlink':
				str = 'Please input URL:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="txt" />'+
					(selection ? '' : '<br />Input Link Text:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="txt" />');
				break;
			case 'forecolor':
			...
			case 'free':
				...
				var lang = {'quote' : 'Please input quote', 'code' : 'Please input code', 'hide' : 'Please input hide content', 'free' : 'Please input free information'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="txt" checked="checked" />Only when replied to shown<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="txt" />Only when credit is higher than <input type="text" size="3" id="' + ctrlid + '_param_2" class="txt" /> to show' : '');
				break;
			case 'table':
				str = 'Rows: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="txt" /> &nbsp; Colums: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="txt" /><br />Width: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="txt" /> &nbsp; Background Color: <input type="text" id="' + ctrlid + '_param_4" size="2" class="txt" onclick="showColorBox(this.id, 2)" />';
				break;
			case 'audio':
				str = 'Please input music URL:<br /><input type="text" id="' + ctrlid + '_param_1" class="txt" value="" style="width: 245px;" />';
				break;
			case 'video':
				str = 'Please input video URL:<br /><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 245px;" class="txt" /><br />Width: <input id="' + ctrlid + '_param_2" size="5" value="400" class="txt" /> &nbsp; Height: <input id="' + ctrlid + '_param_3" size="5" value="300" class="txt" />';
				break;
			case 'flash':
				str = 'Please input Flash URL:<br /><input type="text" id="' + ctrlid + '_param_1" class="txt" value="" style="width: 245px;" />';
				break;
			default:
				...
				for(var i = 1; i <= params; i++) {
					if(i != params || !haveSel) {
						str += (promptlang[i - 1] ? promptlang[i - 1] : 'Please input the ' + i + 'th parameter:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="txt" />' + (i < params ? '<br />' : '');
					}
				}
				break;
		}
		...
		menu.innerHTML = '<div class="popupmenu_option">' + str + '<br /><center><input type="button" id="' + ctrlid + '_submit" value="Submit" /> &nbsp; <input type="button" onClick="hideMenu()" value="Cancel" /></center></div>';
...
function uploadAttach(curId, statusid, prefix) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('Uploading finished! Success: ' + UPLOADCOMPLETE + ', Fail: ' + UPLOADFAILED + ', ' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog('Uploading finished!', 'notice');
			}
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showDialog('Sorry, not support this extension');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showDialog('Please select image file(' + imgexts + ')');
		return;
	}

	$(prefix + 'cpdel_' + id).innerHTML = '<a href="###" class="deloption" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">Delete</a>';
...
function updateattachnum(type) {
	...
	if(num) {
		$(editorid + '_cmd_' + type).title = 'contain ' + num + (type == 'image' ? ' image attachment(s)' : ' attachment(s)');
		$(editorid + '_cmd_' + type + '_notice').style.display = '';
	} else {
		$(editorid + '_cmd_' + type).title = type == 'image' ? 'Image' : 'Attachment';
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		showDialog('Uploading Failed:' + STATUSMSG[arr[2]]);
	}
}
