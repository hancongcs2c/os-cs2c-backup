...
function copycode(obj) {
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'The code has been copied to clipboard');
}
...
function zoom(obj, zimg) {
	...
	if(!menu) {
		...
		$(imgid).onload = function() {
			...
			menu.innerHTML = '<div class="zoominner"><p id="' + menuid + '_ctrl"><span class="right"><a href="' + zimg + '" class="imglink" target="_blank" title="Open in new window">Open in new window</a><a href="javascipt:;" id="' + menuid + '_adjust" class="imgadjust" title="Actual size">Actual size</a><a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Close">Close</a></span>Use Mouse Wheel zoom picture</p><div align="center" onmousedown="zoomclick=1" onmousemove="zoomclick=2" onmouseup="if(zoomclick==1) hideMenu()"><img id="' + zoomid + '" src="' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '"></div></div>';
			$('append_parent').appendChild(menu);
...
function setanswer(pid){
	if(confirm('Choose this reply as "The Best Answer"?')){
		if(BROWSER.ie) {
			doane(event);
		}
...
function submithandle_fastpost(locationhref) {
	...
	if(pid != -1) {
		...
		$('fastpostreturn').innerHTML = 'In this forum, your thread will be shown after audit';
	}
}
...
function creditconfirm(v) {
	return confirm('Download needs ' + v + ', continue?');
}